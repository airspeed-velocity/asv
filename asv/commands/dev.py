...
class Dev(Run):
    ...
    def run_from_conf_args(cls, conf, args, **kwargs):
        return cls.run(conf, bench=args.bench, machine=args.machine,
                       env_spec=args.env_spec, **kwargs)

    @classmethod
    def run(cls, conf, bench=None, env_spec=None, machine=None,
            _machine_file=None):
        if not env_spec:
            env_spec = ["existing:same"]
        return super(cls, Dev).run(conf=conf, bench=bench, show_stderr=True,
                                   quick=True, env_spec=env_spec,
                                   machine=machine, dry_run=True,
