...
from .. import util
from ..plugins.dumbenv import DumbEnv
from ..plugins.dumbrepo import DumbRepo
...
class Run(Command):
    ...
    def setup_arguments(cls, subparsers):
        ...
        parser.add_argument(
            "--dumb", "-d", action="store_true",
            help="""Do a "dumb" run, where no environment is created
            and all dependencies as well as the project being bechmarked
            are assumed to be already importable.""")
        parser.add_argument(
    ...
    def run_from_conf_args(cls, conf, args):
        return cls.run(
            ...
            show_stderr=args.show_stderr, quick=args.quick, dumb=args.dumb,
    ...
    def run(cls, conf, range_spec="master", steps=0, bench=None, parallel=1,
            show_stderr=False, quick=False, profile=False, dumb=False,
            machine_defaults=False, _machine_file=None, _returns={}):
        params = {}
        ...
        benchmarks = Benchmarks(conf, regex=bench,
                                force_env=DumbEnv() if dumb else None,
                                force_repo=DumbRepo() if dumb else None)
        if len(benchmarks) == 0:
        ...
        if dumb:
            benchmarks.run_benchmarks(
                DumbEnv(), show_stderr=show_stderr, quick=quick,
                profile=profile)
            return
