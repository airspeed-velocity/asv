...
class Find(Command):
    ...
    def run(
        ...
        def do_benchmark(i):
            ...
            log.info("For {0} commit {1}:".format(conf.project, commit_name))

            env.install_project(conf, repo, commit_hash)

            res = run_benchmarks(
                benchmarks, env, show_stderr=show_stderr, launch_method=launch_method
            )
        ...
        def do_search(lo, hi):
            ...
            log.info("Testing {0}".format(draw_graph(lo, mid, hi, len(commit_hashes))))
