...
def add_python(parser):
    parser.add_argument(
        "--python", type=str,
        default="same",
        help="""Specify a Python interpreter in which to run the benchmarks.  By
        default, uses the same Python interpreter that asv is using.
        It may be an executable to be searched for on the $PATH, an
        absolute path, or the special value "same" which will use the
        same Python interpreter that asv is using.  This "same"
        interpreter must have the benchmarked project already
