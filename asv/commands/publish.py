...
def check_benchmark_params(name, benchmark):
    ...
    msg = "Information in benchmarks.json for benchmark %s is malformed" % (name)
    if (
        not isinstance(benchmark['params'], list) or
...
class Publish(Command):
    ...
    def run(cls, conf, range_spec=None, pull=True):
        ...
        def copy_ignore(src, names):
            ...
            ignore = [
                fn for fn in names if (
                    os.path.basename(src).lower() == 'vendor' and
                    not fn.lower().endswith('.js') and not fn.lower().endswith('.css')
                )
            ]
            return ignore
        ...
        with log.indent():
            ...
            for results in cls.iter_results(conf, repo, range_spec):
                ...
                branches_for_commit = [
                    branch
                    for branch, commits in branches.items() if results.commit_hash in commits
                ]
